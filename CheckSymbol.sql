...
CREATE VIEW "v01" AS SELECT * FROM "main"."TrendOIpre" WHERE "pVol" >= 0  AND "pOI" >= 0  AND "pChgP" <= 0;
CREATE VIEW "TrendOIpre2" as
SELECT OI1pre.Series, ROUND((OI1pre.Value - OI2.Value)*100/OI2.Value,1) as pVol,(OI1pre.Vol - OI2.Vol)*100/OI2.Vol as pFvol,(OI1pre.OI - OI2.OI)*100/OI2.OI as pOI, OI1pre.pChgP,
OI1pre.PriceR, OI1pre.Pchange, Round(OI1pre.Value/100000,1) as oi1Value
FROM OI1pre 
Join OI2 ON OI1pre.Series = OI2.Series;
CREATE VIEW "TrendOIpre-ckh" as
SELECT * FROM TrendOIpre
WHERE 
...
CREATE VIEW xO1pre AS 
SELECT ssfOI.Series,Close,Pchange,round((Close-Open)*100/Open,2) as PriceR, round((Close-Open)*100/(sPrice.Open),2) as pChgP, 
1 as FVratio,
...
FROM ssfOI 
JOIN sPrice On replace(sPrice.Series,' <XD>','') = ssfOI.Series
...
AND  sPrice.TrdDate = 
(SELECT DISTINCT replace(TrdDate,' 00:00:00','') FROM ssfOI ORDER by TrdDate DESC  LIMIT 1 OFFSET 0)
--AND  replace(sNVDR.TrdDate,' 00:00:00','')  = 
--(SELECT DISTINCT replace(TrdDate,' 00:00:00','') FROM ssfOI ORDER by TrdDate DESC  LIMIT 1 OFFSET 0);
CREATE VIEW xOI2 AS 
SELECT ssfOI.Series,Close,Pchange,round((Close-Open)*100/Open,2) as PriceR, round((Close-Open)*100/(sPrice.Open),2) as pChgP, round(Net/Vol/1000,1) as FVratio,
Value,Vol,OI, Net as Fnet--, (round(round((ssfOI.OI),2)/((ssfOI.Vol)),2)) as OIratio

FROM ssfOI 
JOIN sPrice On replace(sPrice.Series,' <XD>','') = ssfOI.Series
 JOIN "sNVDR" On "sNVDR".Symbol = ssfOI.Series
WHERE ssfOI.TrdDate =  
...
JOIN "sNVDR" On "sNVDR".Symbol = ssfOI.Series
WHERE ssfOI.TrdDate = 
(SELECT DISTINCT TrdDate FROM ssfOI ORDER by TrdDate DESC  LIMIT 1 OFFSET 2)
AND  sPrice.TrdDate = 
(SELECT DISTINCT TrdDate FROM ssfOI ORDER by TrdDate DESC  LIMIT 1 OFFSET 2)
AND  "sNVDR".TrdDate = 
(SELECT DISTINCT TrdDate FROM ssfOI ORDER by TrdDate DESC  LIMIT 1 OFFSET 2);
...
JOIN "sNVDR" On "sNVDR".Symbol = ssfOI.Series
WHERE ssfOI.TrdDate = 
(SELECT DISTINCT TrdDate FROM ssfOI ORDER by TrdDate DESC  LIMIT 1 OFFSET 3)
AND  sPrice.TrdDate = 
(SELECT DISTINCT TrdDate FROM ssfOI ORDER by TrdDate DESC  LIMIT 1 OFFSET 3)
AND  "sNVDR".TrdDate = 
(SELECT DISTINCT TrdDate FROM ssfOI ORDER by TrdDate DESC  LIMIT 1 OFFSET 3);
CREATE VIEW "xTrendOIpre" as
SELECT OI1pre.Series, ROUND((OI1pre.OIratio - OI2.OIratio)*100/OI2.OIratio,1) as pOIRatio, 
ROUND(ROUND((OI2.OI - OI3.OI),2)/OI2.Vol,2)*100 as rOV2,ROUND(ROUND((OI1pre.OI - OI2.OI),2)/OI1pre.Vol,2)*100 as rOV1,
ROUND(ROUND(ROUND((OI1pre.OI - OI2.OI),2)/OI1pre.Vol,2)*100-ROUND(ROUND((OI2.OI - OI3.OI),2)/OI2.Vol,2)*100,1) as dOV,
ROUND((OI1pre.Value - OI2.Value)*100/OI2.Value,1) as pVol,
(OI1pre.OI - OI2.OI)*100/OI2.OI as pOI, OI1pre.pChgP,
sumN,
Round(OI3.Value/100000,1) as V3,Round(OI2.Value/100000,1) as V2,Round(OI1pre.Value/100000,1) as V1, 
round(OI4.Fnet/1000000,1) as Fnet4,round(OI3.Fnet/1000000,1) as Fnet3,round(OI2.Fnet/1000000,1) as Fnet2,round(OI1pre.Fnet/1000000,1) as Fnet1, 
OI3.Vol as VF3,OI2.Vol as VF2,OI1pre.Vol as VF1, OI3.OI as o3,OI2.OI as o2,OI1pre.OI as o1, 
--sPrice.Change, ROUND(sPrice.Close-sPrice.Open,2) as rPrice, sPrice.Close
OI1pre.PriceR, OI1pre.Pchange
FROM OI1pre 
Join OI2 ON OI1pre.Series = OI2.Series
Join OI3 ON OI3.Series = OI1pre.Series
Join OI4 ON OI4.Series = OI1pre.Series
Join sNVDRsum ON sNVDRsum.Symbol = OI1pre.Series
--WHERE v1>1.5
...
--pFVrate DESC
--pOI DESC;
CREATE VIEW "xTrendOI" as
SELECT OI1.Series, ROUND((OI1.OIratio - OI2.OIratio)*100/OI2.OIratio,1) as pOIRatio, 
ROUND(ROUND((OI2.OI - OI3.OI),2)/OI2.Vol,2)*100 as rOV2,ROUND(ROUND((OI1.OI - OI2.OI),2)/OI1.Vol,2)*100 as rOV1,
ROUND(ROUND(ROUND((OI1.OI - OI2.OI),2)/OI1.Vol,2)*100-ROUND(ROUND((OI2.OI - OI3.OI),2)/OI2.Vol,2)*100,1) as dOV,
ROUND((OI1.Value - OI2.Value)*100/OI2.Value,1) as pVol,
(OI1.OI - OI2.OI)*100/OI2.OI as pOI, OI1.pChgP,
sumN,
...
--ORDER by FNrate DESC,FTrate DESC;
CREATE VIEW "TrendOIpre" as
SELECT OI1pre.Series, ROUND((OI1pre.OIratio - OI2.OIratio)*100/OI2.OIratio,1) as pOIRatio, 
Round((Round((OI1pre.OI-OI2.OI),2)*10/OI1pre.Vol)-(Round((OI2.OI-OI3.OI),2)*10/OI2.Vol),1) as pOIC,
(OI1pre.OI - OI2.OI)*100/OI2.OI as pOI,
ROUND((OI1pre.Value - OI2.Value)*100/OI2.Value,1) as pVal,
(OI1pre.Vol - OI2.Vol)*100/OI2.Vol as pVol,
sumN,
Round(OI3.Value/100000,1) as V3,Round(OI2.Value/100000,1) as V2,Round(OI1pre.Value/100000,1) as V1, 
round(OI4.Fnet/1000000,1) as Fnet4,round(OI3.Fnet/1000000,1) as Fnet3,round(OI2.Fnet/1000000,1) as Fnet2,round(OI1pre.Fnet/1000000,1) as Fnet1, 
OI3.Vol as VF3,OI2.Vol as VF2,OI1pre.Vol as VF1, OI3.OI as o3,OI2.OI as o2,OI1pre.OI as o1, 
...
Join OI4 ON OI4.Series = OI1pre.Series
Join sNVDRsum ON sNVDRsum.Symbol = OI1pre.Series
--WHERE OI2.OI <OI1pre.OI 
--and OI2.Vol < OI1pre.Vol
--v1>1.5
--sPrice.TrdDate=(SELECT DISTINCT TrdDate FROM ssfOI ORDER by TrdDate DESC  LIMIT 1 OFFSET 0)
...
FROM TrendOI 
 ...
 --AND
 --TrendOI.TrdDate = (SELECT DISTINCT TrdDate FROM ssfOI ORDER by TrdDate DESC  LIMIT 1 OFFSET 0);
