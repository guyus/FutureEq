...
df['value'] = df['value'].apply(lambda x: x*float(multi))
#print(df['trddate'][0])
a = [0]
...
sql = "select * from spriced where trddate = '"+ df['trddate'].astype(str)[0] + "'"
print(sql)
rst = pd.read_sql(sql, con=engine)
print(len(rst.axes[0]))
if len(rst.axes[0]) == 0:df.to_sql(name='spriced', con=engine, if_exists='append')
p2 = pd.read_sql("select * from spriced where trddate = '"+ pd.to_datetime('today').strftime('%Y-%m-%d') + "'", con=engine)
print(p2)
len(df)