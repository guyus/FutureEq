...
df=pd.read_html('https://classic.set.or.th/set/nvdrbystock.do?sort=total&type=value&language=en&country=US&date=14%2F12%2F2021')
...
where snvdr.TrdDate = (SELECT DISTINCT TrdDate FROM ssfOI ORDER by TrdDate DESC  LIMIT 1 OFFSET 1)"
cur.execute(sqlAvg)

sqlAvg = "UPDATE ssfoi \
SET Avg5 = (SELECT round(avg(n1.Net)) FROM sNVDR n1 WHERE (n1.Symbol = ssfOI.Series AND n1.TrdDate > (SELECT DISTINCT TrdDate FROM ssfOI ORDER by TrdDate DESC  LIMIT 1 OFFSET 6))) \
, Avg10 = (SELECT round(avg(n1.Net)) FROM sNVDR n1 WHERE (n1.Symbol = ssfOI.Series AND n1.TrdDate > (SELECT DISTINCT TrdDate FROM ssfOI ORDER by TrdDate DESC  LIMIT 1 OFFSET 11))) \
, ftavg5 = (SELECT round(avg(n1.ft),1) FROM sNVDR n1 WHERE (n1.Symbol = ssfOI.Series AND n1.TrdDate > (SELECT DISTINCT TrdDate FROM ssfOI ORDER by TrdDate DESC  LIMIT 1 OFFSET 6))) \
, ftavg10 = (SELECT round(avg(n1.ft),1) FROM sNVDR n1 WHERE (n1.Symbol = ssfOI.Series AND n1.TrdDate > (SELECT DISTINCT TrdDate FROM ssfOI ORDER by TrdDate DESC  LIMIT 1 OFFSET 11))) \
WHERE ssfOI.TrdDate = (SELECT DISTINCT TrdDate FROM ssfOI ORDER by TrdDate DESC  LIMIT 1 OFFSET 1)"
cur.execute(sqlAvg)
